...
public sealed partial class MainWindow : Window {
    ...
    public MainWindow() {
        ...
        LanguagePicker.ItemsSource = new[] { "简体中文", "English" };
        LanguagePicker.SelectedIndex = settings.Language == "en" ? 1 : 0;
    ...
    private bool IsEnglish => LanguagePicker.SelectedIndex == 1;
    ...
    private void UpdateState() => DispatcherQueue.TryEnqueue(() => {
        StatusText.Text = server.Connected ? T("手机已连接", "Phone connected") : server.Running ? T("等待手机连接", "Waiting for phone") : T("准备就绪", "Ready");
        AddressText.Text = server.Running ? $"{LocalIPv4()}:{DefaultPort}" : T("将电脑声音无线播放到手机", "Play computer audio on your phone");
        ToggleButton.Content = server.Running ? T("停止服务", "Stop") : T("开始共享声音", "Start sharing");
        DevicePicker.IsEnabled = !server.Running;
    });

    private void LanguageChanged(object sender, SelectionChangedEventArgs e) { if (!initialized) return; settings.Language = IsEnglish ? "en" : "zh"; settings.Save(); ApplyLanguage(); }
    ...
    private void ApplyLanguage() { DevicePicker.PlaceholderText = T("选择电脑输出设备", "Select computer output"); HistoryTitle.Text = T("最近连接", "Recent connections"); ClearHistoryButton.Content = T("清空", "Clear"); FooterText.Text = T("无需虚拟声卡 · WASAPI 系统音频捕获 · 局域网传输", "No virtual driver · WASAPI capture · Local network"); UpdateState(); }
