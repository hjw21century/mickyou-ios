...
internal sealed class SpeakerServer {
    ...
    private void StartCapture() { capture?.StopRecording(); capture?.Dispose(); captureDevice?.Dispose(); using var devices = new MMDeviceEnumerator(); captureDevice = string.IsNullOrWhiteSpace(deviceId) ? devices.GetDefaultAudioEndpoint(DataFlow.Render, Role.Multimedia) : devices.GetDevice(deviceId); capture = new WasapiLoopbackCapture(captureDevice); capture.DataAvailable += (_, e) => ProcessAudio(e.Buffer.AsSpan(0, e.BytesRecorded), capture.WaveFormat); capture.RecordingStopped += (_, _) => { Connected = false; StateChanged?.Invoke(); }; capture.StartRecording(); }
