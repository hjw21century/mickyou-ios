    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 28) {
                ...
                Text(server.connected ? t("手机已连接", "Phone connected") : server.running ? t("等待手机连接", "Waiting for phone") : t("准备就绪", "Ready")).font(.title.bold())
                Text(server.running ? "\(server.address):\(server.port)" : t("将电脑声音无线播放到手机", "Play computer audio on your phone")).foregroundStyle(.secondary)
                HStack {
                    Text(t("端口", "Port")).foregroundStyle(.secondary)
                ...
                Button { server.running ? server.stop() : server.start() } label: { Label(server.running ? t("停止服务", "Stop") : t("开始共享声音", "Start sharing"), systemImage: server.running ? "stop.fill" : "play.fill").frame(width: 220).padding(.vertical, 8) }
